    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          describe("mintNft", function () {
              ...
              it("shifts the token URI to lower when the price doesn't surpass the highvalue", async () => {
                  const highValue = ethers.parseEther("100000000") // 100,000,000 dollar per ether
                  txResponse = await dynamicSvgNft.mintNft(highValue)
                  txResponse.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
          ...
          describe("tokenURI", function () {
              it("Reverts if the `tokenId` doesn't have a current owner", async () => {
